...
import { User, Brain, Moon, AlertTriangle, Thermometer, Send, Clock, Sun, Award, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function DailyParametersMobile() {
  ...
  const renderSingleSelectButton = (
    ...
    <button
      ...
      className={`w-full flex items-center px-3 sm:px-4 py-3 sm:py-4 space-x-2 sm:space-x-3 rounded-xl transition ${mainParam === id ? activeColor : "bg-gray-700"}`}
    >
      <Icon className="w-5 h-5 sm:w-6 sm:h-6" fill={mainParam === id ? "#fff" : "none"} stroke="#fff" strokeWidth={2} />
      <span className="text-xs sm:text-sm md:text-base text-white">{label}</span>
    </button>
  );

  const formattedDate = selectedDate.format("dddd, DD MMMM").split(" ").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
  ...
  return (
    <div className="min-h-screen bg-[#0e0e10] text-white px-3 sm:px-4 py-4 sm:py-6">
      ...
      <div className="flex items-center gap-2 mb-4">
        <button onClick={prevDay} className="p-1 rounded bg-[#1f1f22]"><ChevronLeft className="w-4 h-4" /></button>
        <span className="text-xs text-gray-300">{formattedDate}</span>
        <button onClick={nextDay} className="p-1 rounded bg-[#1f1f22]"><ChevronRight className="w-4 h-4" /></button>
      </div>
