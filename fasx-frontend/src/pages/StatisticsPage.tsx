...
export default function StatsPage() {
  ...
  const TableSection: React.FC<{table:any,index:number}> = ({table,index}) => {
    ...
    return (
      <div className="bg-[#1a1a1d] p-5 rounded-2xl shadow-lg max-w-full overflow-x-auto">
          ...
          <div style={{minWidth:computedMinWidth}} className="transition-all duration-300">
            ...
            <div className="flex bg-[#222] border-b border-[#2a2a2a] sticky top-0 box-border z-10">
              ...
              {filteredMonths.map((m,idx)=>(
                <div key={m+"-h-"+idx} className="p-3 text-center box-border font-medium flex-none" style={{width:colWidth}}>
                  {m}
                </div>
              ))}
                ...
                <div key={j} className="flex border-t border-[#2a2a2a] hover:bg-[#252525]/60 transition">
                  ...
                  {filteredMonths.map((val:number|string,k:number)=>(
                    <div key={k} className="p-3 text-center box-border flex-none" style={{width:colWidth}}>
                      {table.title==="Параметры дня"?row.months[k]:formatTimeSafe(row.months[k])}
                    </div>
                  ))}
              ...
              {table.title!=="Параметры дня" && (
                <div className="flex border-t border-[#2a2a2a] bg-[#222] font-semibold">
                  ...
                  {filteredMonths.map((_,idx)=>(
                    <div key={idx} className="p-3 text-center flex-none" style={{width:colWidth}}>{sumColumn(table.data,idx)}</div>
                  ))}
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-gray-200 p-6">
      {/* главный контейнер — ограниченная ширина */}
      <div className="max-w-5xl mx-auto space-y-6 px-4">
        ...
        {[
          ...
          {
            title: "Выносливость",
            data: filteredEnduranceZones.map(z=>({param:z.zone,months:z.months,total:z.total,color:z.color})),
          },
          {
