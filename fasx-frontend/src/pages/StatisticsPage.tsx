      ...
      <div className="space-y-0.5">
        {payload.map((p: any, i: number) => (
          <div key={i} className="flex justify-between gap-2 items-start">
            ...
            <span className="font-mono text-right min-w-[55px]" style={{ color: p.fill }}>{formatValue(p.value)}</span>
          </div>
        ))}
      </div>
          ...
          <div>
            {table.data.map((row: any, j: number) => (
              <div key={j} className="flex border-t border-[#2a2a2a] hover:bg-[#252525]/60 transition">
                ...
                <div className="p-3 text-center bg-[#1f1f1f] flex-none" style={{ width: totalWidth }}>{row.total}</div>
              </div>
            ))}
          </div>
        </div>
