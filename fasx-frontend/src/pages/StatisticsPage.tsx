...
export default function StatsPage() {
  ...
  const distanceByType = [
    { type: "Лыжи / скейтинг", distance: [40,45,30,0,0,0,0,0,10,20,30,25] },
    { type: "Лыжи, классика", distance: [30,35,25,0,0,0,0,0,0,10,20,15] },
    { type: "Роллеры, классика", distance: [0,0,0,30,50,60,70,50,40,30,20,0] },
    { type: "Роллеры, скейтинг", distance: [0,0,0,20,60,70,80,60,50,30,20,0] },
    { type: "Велосипед", distance: [0,0,0,10,20,30,40,30,20,10,0,0] },
  ];
  ...
  const setWeek = () => { setStartPeriod(dayjs().startOf('isoWeek').toDate()); setEndPeriod(dayjs().endOf('isoWeek').toDate()); };
  const setMonth = () => { setStartPeriod(dayjs().startOf('month').toDate()); setEndPeriod(dayjs().endOf('month').toDate()); };
  const setYear = () => { setStartPeriod(dayjs().startOf('year').toDate()); setEndPeriod(dayjs().endOf('year').toDate()); };
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-gray-200 p-6 w-full">
      <div className="w-full space-y-8">
        ...
        <div className="bg-[#1a1a1d] p-5 rounded-2xl shadow-lg overflow-hidden">
          <h2 className="text-lg font-semibold mb-4 text-gray-100">Зоны выносливости</h2>
          <div className="h-64 bg-[#1a1a1d]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={filteredMonths.map((month,i) => {
                  const row: any = { month };
                  filteredEnduranceZones.forEach(z => row[z.zone] = z.months[i]);
                  return row;
                })}
                barGap={0}
              >
            ...
            <div className="h-64 bg-[#1a1a1d]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={filteredMonths.map((month,i)=>{
                    const row: any = { month };
                    filteredDistanceTypes.forEach(d => row[d.type]=d.months[i]);
                    return row;
                  })}
                  barGap={0}
                >
                  ...
                  {filteredDistanceTypes.map(d=>(
                    <Bar key={d.type} dataKey={d.type} stackId="a" fill={distanceColors[d.type]} radius={[4,4,0,0]} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </div>
        ...
        {/* Таблицы (выносливость и активность) */}
        <div className="space-y-6">
          ...
          <div className="bg-[#1a1a1d] p-5 rounded-2xl shadow-lg overflow-x-auto">
            ...
            <table className="w-full min-w-[900px] text-sm border-collapse bg-[#1a1a1d]">
              <thead className="bg-[#222] text-gray-400 text-left">
                <tr>
                  <th className="p-3 sticky left-0 bg-[#222]">Зона</th>
          ...
          <div className="bg-[#1a1a1d] p-5 rounded-2xl shadow-lg overflow-x-auto">
            ...
            <table className="w-full min-w-[900px] text-sm border-collapse bg-[#1a1a1d]">
              <thead className="bg-[#222] text-gray-400 text-left">
                <tr>
                  <th className="p-3 sticky left-0 bg-[#222]">Тип активности</th>
                  {filteredMonths.map(m => <th key={m} className="p-3 font-medium text-center">{m}</th>)}
                  <th className="p-3 font-medium text-center bg-[#1f1f1f]">Всего</th>
                </tr>
              </thead>
