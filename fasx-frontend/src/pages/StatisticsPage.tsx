...
// Толтип //
 ...
 const TableSection: React.FC<{ table: any; index: number }> = ({ table, index }) => {
   const leftWidth = 200; // ширина первой колонки (название параметра)
   const totalWidth = 80; // ширина колонки "Всего"
   const containerRef = useRef<HTMLDivElement>(null);

   // Вычисляем динамическую ширину столбцов
   const colWidth = React.useMemo(() => {
     if (!containerRef.current) return 80;
     const availableWidth = containerRef.current.clientWidth - leftWidth - totalWidth;
     return Math.max(80, Math.floor(availableWidth / filteredMonths.length));
   }, [filteredMonths.length, containerRef.current?.clientWidth]);

   return (
     <div className="bg-[#1a1a1d] p-5 rounded-2xl shadow-lg" ref={containerRef}>
       ...
       <div ref={scrollRefs[index]} className="overflow-x-auto" onScroll={e => handleScroll(e, index)}>
         <div style={{ minWidth: leftWidth + totalWidth + colWidth * filteredMonths.length }} className="transition-all">
