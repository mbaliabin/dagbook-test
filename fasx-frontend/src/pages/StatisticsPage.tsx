  ...
  const generateData = () => {
    ...
    if (reportType === "Выносливость") {
      ...
      chartData = labels.map((label) => {
        const obj: any = { label };
        zones.forEach((z) => obj[z] = Math.floor(Math.random() * 60));
        return obj;
      });
      tableData = [
        {
          type: "Выносливость",
          values: labels.map(() => Math.floor(Math.random() * 60)),
          total: Math.floor(Math.random() * 300),
        }
      ];
    } else {
      const maxValues = reportType === "Общее расстояние" ? { run: 10, ski: 15, bike: 20, swim: 5 } : { run: 60, ski: 90, bike: 120, swim: 30 };
      chartData = labels.map((label) => ({
        label,
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-[#1a1a1d] p-6 rounded-2xl shadow-md mb-10">
          <h2 className="text-lg font-semibold mb-3">
            Тип тренировки ({reportType === "Общее расстояние" ? "км" : reportType === "Длительность" ? "мин" : "Зоны"})
          </h2>
            ...
            <table className="w-full text-sm text-gray-300 border-collapse border border-gray-800 rounded-xl overflow-hidden">
              <thead className="text-gray-400 bg-gradient-to-b from-[#18191c] to-[#131416]">
                <tr>
                  ...
                  {labels.map((m) => <th key={m} className="py-2 px-2 text-center border-r border-gray-700/70">{m}</th>)}
